...
//어떤 Controller 객체가 검색되더라도 같은 코드로 실행하려면 모든 컨트롤의 최상위 인터페이스 필요
...
public class LoginController {
	...
	public String loginView(@ModelAttribute("user") UserVO vo) {
		// 스프링 컨테이너가 생성하는 객체의 이름은 클래스 이름 (UserVO)의 첫글자를 소문자로 변경한 이름이 자동으로 설정됨
		// 따라서 UserVO객체의 변수에 접근할 때 ${userVO.변수명}으로 사용한 것이다
